...
class Zombie:
    ...
    def is_zombie_ball_count_higher(self):
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('좀비의 공 카운트가 더 높은가?', self.is_zombie_ball_count_higher)

        a1 = Action('Set target location', self.set_target_location, 500, 500)
        a2 = Action('Move to', self.move_to)
        move_to_target_location = Sequence('Move to target location', a1, a2)  # 타겟 위치로 이동

        a3 = Action('Set random location', self.set_random_location)
        wander = Sequence('Wander', a3, a2)  # 랜덤하게 배회
        a4 = Action('소년에게 접근', self.move_to_boy)
        chase_boy = Sequence('소년을 추적', c1, c2, a4)  # 소년 근처에 있고 좀비의 공이 더 많을 때 추적


        a5 = Action('소년으로부터 멀어짐', self.runaway_from_boy)
        runaway_boy = Sequence('소년으로부터 도망', c1, a5)  # 소년 근처에 있고 좀비의 공이 적을 때 도망

        a6 = Action('순찰 위치 가져오기', self.get_patrol_location)
        patrol = Sequence('순찰', a6, a2) # 정해진 곳 순찰

        root = Selector('좀비 행동 선택', chase_boy, runaway_boy, patrol)
